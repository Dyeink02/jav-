...
class CloudflareBypass {
    ...
    if (!this.page) {
      ...
      if (this.puppeteerPool) {
        try {
          ...
          logger.error(`重新从共享池获取页面实例失败：${poolError instanceof Error ? poolError.message : String(poolError)}`);
  ...
  public async executeAjax(url: string): Promise<any> {
    ...
    try {
      ...
      try {
        ...
        logger.warn(
          `executeAjax: 解析当前页面 URL 失败: ${error instanceof Error ? error.message : String(error)}`
        );
      }
      ...
      if (isBrowserErrorPage || !currentUrlObj || ajaxUrlObj.hostname !== currentHostname) {
        ...
        try {
          ...
          logger.warn(`AJAX 预导航的 domcontentloaded 阶段失败，改用 load：${navError instanceof Error ? navError.message : String(navError)}`);
          await page.goto(`${ajaxUrlObj.protocol}//${ajaxUrlObj.hostname}/`, {
            waitUntil: 'load',
      ...
      const totalTime = Date.now() - ajaxStartTime;
      logger.error(`页面内 AJAX 请求失败（耗时：${Math.round(totalTime/1000)} 秒）`);
      logger.error(`错误详情：${error instanceof Error ? error.message : String(error)}`);
      ...
      try {
        ...
        logger.warn(`检查浏览器进程状态失败：${processError instanceof Error ? processError.message : String(processError)}`);
      }
  ...
  private async waitForCloudflareChallenge(): Promise<void> {
    ...
    try {
      ...
      let timeoutReason = '页面仍未完成验证';
      if (hasAgeVerification) {
        timeoutReason = '页面停留在年龄验证页';
      } else if (hasError) {
        timeoutReason = '页面停留在 Cloudflare 错误页';
      } else if (hasChallenge) {
        timeoutReason = '页面仍停留在 Cloudflare 挑战页';
      } else if (pageTitle) {
        timeoutReason = `页面标题仍为“${pageTitle}”`;
      }

      throw new Error(`Cloudflare challenge timeout: ${timeoutReason}`);
    }
  }
